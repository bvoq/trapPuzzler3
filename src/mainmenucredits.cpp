...
static ddd mainMenuLevel, mainMenuMoveLevel;
...
static int additionalsfxwidth = 10;
static int startPoint = menuwidth + additionalsfxwidth;
static int menulength = 16;
static int structurewidth = 16;
static int sfxlength = 8, sfxwidth = 15;//24; //looks cool with 31
...
static inline float getMenuTileWidth() {
    return min((getHeight() * 1.) / (grid.size()+2),  (getWidth() * 1.) / (grid[0].size()+2));
}

static ofTrueTypeFont mainMenuFont; static float previousMenuWidthForMainMenuFont = -2; //License see 1.4 http://theleagueof.github.io/licenses/ofl-faq.html
static ofTrueTypeFont mainMenuFontLarge;
