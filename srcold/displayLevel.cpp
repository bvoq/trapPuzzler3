...
static int CIRCLE_RESOLUTION = 50;

//2^4 possibilities of mesh = 16 pre-processing

ofMesh generateMeshTile(float w, float h, float r, ofFloatColor tlc, ofFloatColor trc, ofFloatColor blc, ofFloatColor brc, bool neighL, bool neighR, bool neighU, bool neighD) {
    ofPolyline poly;
...
void drawMonsterEye(int i, int j, float scale, float tScale, deque<deque<int> > & grid, float deathGradient) {
    ...
    ofFill();
    ofSetColor(255-100*deathGradient,255-150*deathGradient,255-150*deathGradient);
    ofPushMatrix();
    ...
    if(deathGradient != 1.) {
        for(float a = 0; a <= 1; a += 0.2) {
            float leftPos = (1.-deathGradient)*(currentWatchY-i) * 1./grid.size() * .25 * scale + scale*.6 * a + scale/2. - scale*.3;
    ...
    ofPushMatrix();
    ofTranslate(0,(1.-deathGradient)*(currentWatchY-i) * 1./grid.size() * .25 * scale);
...
static int fastrandtoothgenseed = 0;
...
//local graphics DP
static pair<vector<vector<ofRectangle> >, vector<vector<cellType> > > rectsDP; //TODO! Reset this on change of width, etc!
static float scaleChange = -1; //In case any of the figures change size due to continous movement, redraw.
