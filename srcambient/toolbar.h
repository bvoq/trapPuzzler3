...
void displayToolBar() {
    if(mode == LEVEL_EDITOR || mode == LEVEL_EDITOR_PLAYING || (!messageBlockingToolbar && !blockMovementDueToWinningAnimation)) {
            ...
            if(toolbarOrientation == BOTTOMTOOLBAR) {
                ...
                if(mode == CREDITS || mode == CONTROL_CHANGE) {
                    ...
                    if(mousetouchX > (ofGetWidth() - 1*toolbarSize + toolbarSize * 0.05) && mousetouchX < (ofGetWidth() - toolbarSize * 0.05)
                       && mousetouchY > ofGetHeight() - toolbarSize + toolbarSize * 0.05 && mousetouchY < ofGetHeight() - toolbarSize * .05) {
                        ofScale(1.05,1.05);
