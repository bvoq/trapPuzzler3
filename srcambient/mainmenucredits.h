...
void displayMainMenu() {
    ...
    //STORY-MODE
    if(getCellType(grid[2][additionalsfxwidth+1]) == PLAYER || (mousetouchY >=  -singleMenuTileWidth + transY && mousetouchY <= transY && mousetouchX >= transX - mainMenuFont.stringWidth("Start Game") && mousetouchX <= transX)) {
        highlightedMainMenuPrintRightAligned("Start Game", singleMenuTileWidth);
    } else {
        unhighlightedMainMenuPrintRightAligned("Start Game", singleMenuTileWidth);
    }
    
    if(isMousePressed && (mousetouchY >=  -singleMenuTileWidth + transY && mousetouchY <= transY && mousetouchX >= transX - mainMenuFont.stringWidth("Start Game") && mousetouchX <= transX)) {
        gotoYOption = 2;
    }
    ...
    if(gotoYOption != -1 && movements.size() == 0) {
        ...
        if(playerID != 1) { //twice because there are three players in total
            movement m = movement(moveGrid, moveGrid, moveEyeGrid, moveEyeGrid,
                                  CHANGE_TO_PLAYER, {}, false, timeForMovement, false, PLAYER_CHANGEMT
                                  );
            changePlayerIdDeterministic(moveGrid,playerID,false);
    ...
    unhighlightedMainMenuPrintCentered("Made by Kevin De Keyser", singleMenuTileWidth);
...
void displayCredits() {
    ...
    unhighlightedMainMenuPrintRightAligned("Kevin De Keyser (@kdkdkch, git:bvoq)",singleMenuTileWidth);
    ofTranslate(0,singleMenuTileWidth*1);
    ...
    ofTranslate(-singleMenuTileWidth*3,+singleMenuTileWidth);
    ofTranslate(0,singleMenuTileWidth*1);
    unhighlightedMainMenuPrintRightAligned("Light mood: Lars Faber (dirtyuludag)",singleMenuTileWidth);
    ofTranslate(0,singleMenuTileWidth*1);
    unhighlightedMainMenuPrintRightAligned("Normal mood: Night Driver (nosoapradio.us)",singleMenuTileWidth);
    /*
    ofTranslate(0,singleMenuTileWidth*1);
    ...
    ofTranslate(-singleMenuTileWidth*4.5, singleMenuTileWidth*grid.size()/4-3*singleMenuTileWidth);
    highlightedMainMenuPrintCentered("The puzzle game whistleblower", singleMenuTileWidth);
    ofPopMatrix();
}
