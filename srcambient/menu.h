...
void displayOldMenu() {
    ...
    static int prevlightmode = -1;
    if(backgroundTile.first.first != getWidth() || backgroundTile.first.second != getHeight() || lightmode != prevlightmode) {
        backgroundTile = {{getWidth(), getHeight()},
    ...
    if(scrollX > 50 && currentLevel-levelCountPerWorld >= 0) backgroundTile.second.draw();
    ofPopMatrix();
    
    for(int i = 0; i < levels.size(); ++i) {
        if((i / levelCountPerWorld == currentWorld-1 && scrollX > getWidth()/10. ) || i / levelCountPerWorld == currentWorld || (i/levelCountPerWorld == currentWorld+1 && scrollX < getWidth()/10.)) {
            ...
            if((!isMousePressed && i == currentLevel) || (isMousePressed && mousetouchX >= positionX && mousetouchX < positionX+w && mousetouchY >= positionY && mousetouchY < positionY + h && i / levelCountPerWorld == currentWorld)) {
